...
class AdbClient:
    def __init__(self, adb_path: str, device_ip: str, candidate_ports: Iterable[int], logger: LogFn = print):
    ...
    def connect(self, host: str, port: int) -> Tuple[bool, str]:
        ...
        ok = result.returncode == 0 and (
            "connected" in text.lower() or "already connected" in text.lower() or not text
        )
        return ok, text
    ...
    def auto_connect(self) -> Optional[str]:
        ...
        for device in devices:
            if device.startswith(("127.0.0.1:", "emulator-", "localhost:")):
                self.device_id = device
                self.log(f"[ADB] Đã dùng device đang có: {device}")
                return device
        ...
        for device in devices:
            if device.startswith(("127.0.0.1:", "emulator-", "localhost:")):
                self.device_id = device
                self.log(f"[ADB] Kết nối thành công: {device}")
                return device
    ...
    def screencap(self) -> np.ndarray:
        ...
        if proc.returncode != 0 or not proc.stdout:
            err = proc.stderr.decode(errors="ignore") if isinstance(proc.stderr, bytes) else str(proc.stderr or "")
    ...
    def start_app(self, package_name: str, activity: str = "") -> None:
        ...
        if activity:
            ...
            result = self.shell("monkey", "-p", package_name, "-c", "android.intent.category.LAUNCHER", "1", timeout=25)
